...
class SubscriptionService:
    ...
    def get_points_username(self, username):
        keys = self._r_conn.keys("slots:" + username + ":*")
        if len(keys) <= 0:
            return None, None
        return keys[0], int(self._r_conn.get(keys[0]))
